...
class Tourney(BaseDbModel):
    ...
    async def get_groups(self, size: int = None) -> List[List["TMSlot"]]:
        size = size or self.group_size
        return split_list(await self.assigned_slots.all().order_by("num"), size)

    async def get_group(self, num: int, size: int = None) -> List["TMSlot"]:
        _all = await self.get_groups(size)
        try:
            return _all[num - 1]
        except IndexError:
            return None
