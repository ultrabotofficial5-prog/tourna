...
class Quotient(commands.AutoShardedBot):
    ...
    async def setup_hook(self) -> None:
      ...
      # Load extensions FIRST and AWAIT them to avoid race conditions with other startup tasks
      await self.__load_extensions()

      for coro_func in on_startup:
        if coro_func.__name__ == "__load_extensions":
            continue
    ...
    def reminders(self) -> Reminders:  # since we use it a lot
        cog = self.get_cog("Reminders")
        if cog is None:
            # Return a dummy object that logs a warning instead of crashing
            class DummyReminders:
                def __init__(self, bot): self.bot = bot
                async def create_timer(self, *args, **kwargs):
                    print(f"WARNING: create_timer called but Reminders cog is not loaded! Args: {args} Kwargs: {kwargs}")
                    return None
            return DummyReminders(self)
        return cog
