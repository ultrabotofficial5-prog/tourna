...
class HelpCommand(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping: Mapping[Cog, List[commands.Command]]):
        ...
        # Build links, filtering out invalid URLs
        links = []
        if config.SERVER_LINK and config.SERVER_LINK.startswith(('http://', 'https://', 'discord')):
            links.append(LinkType("Support Server", config.SERVER_LINK))
        if config.BOT_INVITE and config.BOT_INVITE.startswith(('http://', 'https://', 'discord')):
            links.append(LinkType("Invite Me", config.BOT_INVITE))
        
        await ctx.send(embed=embed, embed_perms=True, view=LinkButton(links) if links else None)
