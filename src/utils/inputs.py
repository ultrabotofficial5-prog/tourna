...
async def channel_input(ctx: Context, check=None, timeout=120, delete_after=False, check_perms=True):
    ...
    try:
        ...
        except (commands.BadArgument, commands.ChannelNotFound):
            raise InputError(f"Channel \"{message.content}\" not found.")
...
async def role_input(ctx: Context, check=None, timeout=120, hierarchy=True, check_perms=True, delete_after=False):
    ...
    try:
        ...
        except (commands.BadArgument, commands.RoleNotFound):
            raise InputError(f"Role \"{message.content}\" not found.")
    except asyncio.TimeoutError:
...
async def member_input(ctx: Context, check=None, timeout=120, delete_after=False):
    ...
    try:
        ...
        try:
            ...
            raise InputError(f"Member \"{message.content}\" not found.")
