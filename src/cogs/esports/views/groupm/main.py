...
class TourneyGroupManager(EsportsBaseView):
    ...
    async def create_roles_channels(self, interaction: discord.Interaction, button: discord.Button):
        ...
        await self.ctx.simple("Group channels and roles creation successfully", 5)
    ...
    async def send_grouplist(self, interaction: discord.Interaction, button: discord.Button):
        ...
        if await self.tourney.assigned_slots.all().count() == 0:
            return await self.ctx.error("Nobody registered yet.", 4)
